...
beforeEach(function () {
    ...
    // Use existing employee - Wynne Sexton from Kharis Hostels
    $this->employee = Employee::find(1);

    // Update employee attributes for test consistency
    ...
    // Use existing leave type but update it for test requirements
    $this->leaveType = LeaveType::find(1);
...
test('it calculates monthly pro rata for new hires', function () {
    // Use existing employee Hunter Fitzpatrick from Kharis Management Consult
    $newEmployee = Employee::find(4);
    $newEmployee->update([
...
test('it handles part time employee adjustments', function () {
    // Use existing employee Hunter Fitzpatrick from Kharis Management Consult
    $partTimeEmployee = Employee::find(4);
    $partTimeEmployee->update([
...
test('it processes year end carry over correctly', function () {
    // Create a balance with 15 days remaining using existing employee
...
test('it respects maximum balance cap', function () {
    // Create a balance with 25 days remaining using existing employee
    $balance = LeaveBalance::create([
        'employee_id' => $this->employee->id,
        'leave_type_id' => $this->leaveType->id,
        'year' => 2023,
...
test('it generates accrual forecast', function () {
    // Create current year balance using existing employee
    $balance = LeaveBalance::create([
        'employee_id' => $this->employee->id,
        'leave_type_id' => $this->leaveType->id,
        'year' => now()->year,
...
test('it handles terminated employees', function () {
    // Use existing employee Hunter Fitzpatrick from Kharis Management Consult
    $terminatedEmployee = Employee::find(4);
    $terminatedEmployee->update([
...
test('it handles different accrual frequencies', function () {
    // Create new leave types with different frequencies using existing company
    $quarterlyType = LeaveType::create([
        'company_id' => $this->company->id,
