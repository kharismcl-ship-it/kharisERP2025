<div>
    ...
    <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex space-x-6 overflow-x-auto">
            @foreach(['occupancy' => 'Occupancy', 'bookings' => 'Bookings', 'revenue' => 'Revenue', 'deposits' => 'Deposits', 'maintenance' => 'Maintenance SLA', 'incidents' => 'Incidents', 'booking-types' => 'Booking Types'] as $tab => $label)
                <button wire:click="$set('activeTab', '{{ $tab }}')"
                        class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                        @if($activeTab === $tab) border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif">
                    {{ $label }}
                </button>
            @endforeach
        </nav>
    ...
    @if($activeTab === 'revenue')
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Total Revenue (This Month)</h3>
                <p class="text-2xl font-semibold">{{ number_format($this->revenueStats['total_revenue'], 2) }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Total Bookings</h3>
                <p class="text-2xl font-semibold">{{ $this->revenueStats['total_bookings'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Avg Booking Value</h3>
                <p class="text-2xl font-semibold">{{ number_format($this->revenueStats['avg_booking_value'], 2) }}</p>
            </div>
        </div>
    @endif
    ...
    @if($activeTab === 'deposits')
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <p class="text-2xl font-semibold text-yellow-600">{{ $this->depositStats['pending'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Collected</h3>
                <p class="text-2xl font-semibold text-green-600">{{ $this->depositStats['collected'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Refunded</h3>
                <p class="text-2xl font-semibold text-blue-600">{{ $this->depositStats['refunded'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Total Held</h3>
                <p class="text-2xl font-semibold">{{ number_format($this->depositStats['total_held'], 2) }}</p>
            </div>
        </div>
    @endif
    ...
    @if($activeTab === 'maintenance')
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <p class="text-2xl font-semibold">{{ $this->maintenanceSla['total'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Resolved</h3>
                <p class="text-2xl font-semibold text-green-600">{{ $this->maintenanceSla['resolved'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-medium text-gray-500 mb-1">Resolved within 48h</h3>
                <p class="text-2xl font-semibold">{{ $this->maintenanceSla['within_48h_pct'] }}%</p>
            </div>
        </div>
    @endif

    <!-- Booking Types Tab -->
    @if($activeTab === 'booking-types')
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <table class="w-full text-sm divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($this->bookingTypeBreakdown as $row)
                        <tr>
                            <td class="px-6 py-3">{{ ucfirst(str_replace('_', ' ', $row->booking_type)) }}</td>
                            <td class="px-6 py-3 text-right font-medium">{{ $row->count }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
