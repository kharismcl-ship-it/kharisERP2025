...
class CsVisitorResource extends Resource
{
     ...
     * public kiosk Livewire component. Keeps a single source of truth.
     *
     * @return Step[]
     */
        ...
        return [
            Step::make('Visitor Type')
                ->schema([
                    ...
                    Placeholder::make('visitor_lookup_hint')
                        ...
                        ->content(function (Get $get) {
                            ...
                            $query = CsVisitor::query()->where('phone', $get('search_phone'));
                            if ($companyId) {
                                $query->withoutGlobalScopes()->where('company_id', $companyId);
                            }
                            $visitor = $query->latest()->first();
                            if ($visitor) {
                ...
                ->afterValidation(function (Get $get, Set $set) {
                    ...
                    $companyId = $get('company_id');
                    $query = CsVisitor::query()->where('phone', $phone);
                    if ($companyId) {
                        $query->withoutGlobalScopes()->where('company_id', $companyId);
                    }
                    $visitor = $query->latest()->first();
